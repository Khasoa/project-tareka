...
function validIdentifier(raw: string): boolean {
  const t = raw.trim();
  if (!t) return false;
  if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(t)) return true;
  const pn = normalizePhoneInput(t);
  const digits = pn.replace(/\D/g, "").length;
...
export function CompanySignInForm() {
  ...
  function validateSignin(): SigninErrors {
    ...
    if (!identifier.trim()) e.identifier = t("companyAuth.errors.identifierRequired");
    else if (!validIdentifier(identifier)) e.identifier = t("companyAuth.errors.invalidContact");
  ...
  async function handleSignin(ev: React.FormEvent) {
    ...
    setSigningIn(true);
    try {
      const payload = buildCompanyLoginPayload(identifier, password);
      await login(payload);
    ...
    <div className="dark flex min-h-screen flex-col items-center justify-center px-5 py-14 bg-background">
      <div className="w-full max-w-[420px]">
        ...
        <div className="mb-9 flex items-center gap-5">
          ...
          {/* Logo links to home — not /for-companies — so clicking always returns to main */}
          <Logo href="/" className="text-xl" variant="chrome" />
        </div>

        <div className="mb-7">
          <p className="text-[11px] font-medium uppercase tracking-[0.12em] text-dim">
            {t("companyAuth.signIn.kicker")}
          </p>
          <h1 className="mt-1 font-heading text-[1.5rem] font-semibold leading-tight tracking-[-0.018em] text-foreground">
        ...
        >
          ...
          <form onSubmit={handleSignin} noValidate className="space-y-4">
            <Field label={t("companyAuth.fields.emailOrPhone")} error={signinErrors.identifier}>
              <input type="text" autoComplete="username"
                placeholder={t("companyAuth.placeholders.emailOrPhone")}
                value={identifier}
                onChange={(e) => { setIdentifier(e.target.value); if (signinErrors.identifier) setSigninErrors((p) => ({ ...p, identifier: undefined })); }}
                className={cn(companyInputCls, signinErrors.identifier ? "ring-1 ring-[rgba(180,90,90,0.5)]" : "focus:ring-[rgba(161,201,152,0.36)]")}
            ...
            <Field label={t("companyAuth.fields.password")} error={signinErrors.password}>
              <div className="relative">
                <input type={showPassword ? "text" : "password"} autoComplete="current-password"
                  placeholder="••••••••"
                  value={password}
                  onChange={(e) => { setPassword(e.target.value); if (signinErrors.password) setSigninErrors((p) => ({ ...p, password: undefined })); }}
                  className={cn(companyInputCls, "pr-10", signinErrors.password ? "ring-1 ring-[rgba(180,90,90,0.5)]" : "focus:ring-[rgba(161,201,152,0.36)]")}
                  style={companyInputStyle}
                />
            ...
            <button type="submit" disabled={signingIn}
              ...
              onMouseEnter={(e) => { if (!signingIn) e.currentTarget.style.background = "#B0D4A8"; }}
              onMouseLeave={(e) => { if (!signingIn) e.currentTarget.style.background = "#A1C998"; }}
            >
              {signingIn ? <><Spinner />{t("companyAuth.signIn.submitting")}</> : t("companyAuth.signIn.submit")}
            </button>
          </form>
        </div>

        <p className="mt-6 text-center text-sm">
          {/* accent-sage-ink resolves to #A1C998 on .dark — legible */}
          <Link href="/company/request-access" className="font-medium text-accent-sage-ink transition-opacity hover:opacity-80">
