...
// ─────────────────────────────────────────────────────────────────────────────
// Role → redirect (used after auto-login post-register)
// ─────────────────────────────────────────────────────────────────────────────
...
export function RegisterForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Step 2: auto-login — backend does not set a cookie on register,
      // so we call the login endpoint immediately with the same credentials.
      ...
      try {
        ...
        // Auto-login failed — fall back to manual sign-in
      ...
      >
        ...
        <header className="mb-9 flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-6">
          ...
          >
            ...
            <form onSubmit={handleSubmit} noValidate className="space-y-4">
              ...
              <Field label="Full name" error={fieldErrors.fullName}>
                <input
                  type="text" autoComplete="name" placeholder="Amara Osei"
                  value={fullName}
                  onChange={(e) => { setFullName(e.target.value); clearField("fullName"); }}
                  className={cn(input, fieldErrors.fullName ? "ring-1 ring-[rgba(180,90,90,0.5)]" : "focus:ring-[rgba(168,191,166,0.36)]")}
                  style={inputStyle}
                />
              </Field>
              ...
              <Field label="Password" error={fieldErrors.password}>
                <div className="relative">
                  <input
                    type={showPassword ? "text" : "password"} autoComplete="new-password" placeholder="At least 8 characters"
                    value={password}
              ...
              <Field label="Confirm password" error={fieldErrors.confirmPassword}>
                <div className="relative">
                  <input
                    type={showConfirm ? "text" : "password"} autoComplete="new-password" placeholder="Repeat your password"
                    value={confirmPassword}
                    onChange={(e) => { setConfirmPassword(e.target.value); clearField("confirmPassword"); }}
                    className={cn(input, "pr-10", fieldErrors.confirmPassword ? "ring-1 ring-[rgba(180,90,90,0.5)]" : "focus:ring-[rgba(168,191,166,0.36)]")}
                    style={inputStyle}
                  />
            ...
            {/*
             * TODO: Wire to real Google OAuth when backend adds /auth/google.
             * Backend should set HttpOnly session cookie and redirect to
             * /auth/callback?redirect=<original_path>. No localStorage tokens.
             */}
            <button
              type="button" aria-label="Continue with Google (not available)"
          ...
          <p className="mt-6 text-center text-sm text-dim">
            ...
            <Link href="/auth/login" className="font-medium transition-opacity hover:opacity-80" style={{ color: "#A8BFA6" }}>
              Sign in
            </Link>
          </p>
        </div>
